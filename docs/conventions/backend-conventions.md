...
대상 경로의 추가 지침은 [루트 읽기 경로](../../AGENTS.md#읽기-경로)에 따라 확인하고 아래에서 현재 단계에 필요한 절만 선택합니다. 소스·테스트의 모듈 진입점을 찾을 때는 [도메인 지도](../domain/README.md#작업-경로와-추가-지침)를 사용합니다. 각 주제의 상세 규칙은 연결된 원본이 소유하며, AI 작업 방식과 완료 명령은 [루트 지침](../../AGENTS.md)이 원본입니다. 이미 읽은 원본으로 되돌아가는 링크는 재조회 요청이 아닙니다.
...
| auth 조회 API·문서 수정 | [도메인 지도](../domain/README.md) → Auth 문서·지침, [Web API](web/http.md#http), [OpenAPI](web/openapi/controllers.md#controllers), [포맷·명명](java/style.md#style). subject 예제에 인증 의미를 부여하지 않음 | `./gradlew test --tests com.example.ApiWorkflowIntegrationTest --tests com.example.OpenApiDocumentationIntegrationTest -PrequireAllTests=true` |
