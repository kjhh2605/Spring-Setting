...
[자동 검사와 리뷰의 역할](testing/evidence.md#evidence)에 따라 유효한 검사 결과를 재사용하고, 자동 검사가 다루지 않은 요구사항·영향과 검사 자체의 적절성을 검토합니다.

1. **전체 범위 파악**: PR 목적·완료 조건과 실제 base/head SHA를 확인합니다. 같은 SHA의 통계·커밋 순서가 이미 제공되면 재사용합니다. 없거나 기준이 바뀌었으면 `git diff --stat <base-sha>...<head-sha>`, `git diff --numstat <base-sha>...<head-sha>`로 변경 범위를, `git log --reverse --format='%h %s' <base-sha>..<head-sha>`로 진행 순서를 확인합니다. 선행 PR과 현재 차이를 구분하고 같은 SHA 기준으로 리뷰합니다.
...
4. **전체 연결 확인**: 모든 묶음을 본 뒤 호출부·공개 계약·구현·설정·테스트가 서로 일치하는지 확인합니다. 모듈 경계와 변경 전파는 [아키텍처 규칙](architecture/modules.md#modules)을 따릅니다. 묶음 사이의 미확인 사항을 해소하고 같은 원인의 지적을 합칩니다.
