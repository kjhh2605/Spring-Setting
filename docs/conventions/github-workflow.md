...
- **크기 점검:** 현재 단위의 미커밋 추가·삭제 합계가 약 500줄에 도달하기 전에, 또는 다음 변경으로 초과할 것이 예상되는 시점에 분리 가능성을 점검합니다. 이번 단위의 staged·unstaged·새 파일을 모두 포함하되 같은 변경을 중복 계산하지 않고, 기존 사용자 변경은 별도로 구분합니다. 산정 기준은 [PR 크기와 AI 리뷰](#pr-크기와-ai-리뷰)를 따릅니다.
- **초과 시 판단:** 독립적으로 검토·검증할 수 있는 부분은 현재 단위로 완결해 커밋하고 나머지는 다음 단위로 옮깁니다. 분리하면 계약·구현·검증이 깨지는 경우에는 계속 구현하기 전에 결합된 대상과 이유를 설명하고 계획을 갱신합니다. 500줄은 점검 기준이며 줄 수만 맞추려고 필요한 코드·테스트를 떼지 않습니다. 이 설명은 별도의 사용자 승인 절차를 추가하지 않습니다.
...
도구가 승인 UI를 제공하면 해당 요청에 위 요약을 담을 수 있습니다. 대화에서 받은 승인과 실행 도구의 권한 검사는 별개이며, 도구가 거부하면 승인 모드·규칙을 임의로 바꾸거나 다른 실행 경로로 우회하지 않습니다. `.rules`의 일치 여부가 행위의 승인 필요성을 결정하지는 않습니다. 정책 선택 이유는 [ADR-004](../adr/004-agent-commits-and-approvals.md)와 [추가 승인 경계](../adr/005-work-loss-and-github-approvals.md)를 참고합니다.
...
조사 결론은 이슈에 남기고, 확정된 구조·정책은 관련 컨벤션에 반영하고 [ADR 작성 규칙](../adr/003-adr-lifecycle.md#작성과-변경-규칙)에 따라 새 ADR로 기록합니다. [현재 ADR 요약](../adr/README.md)과 대체 이력도 함께 갱신합니다. 코드·상시 문서 변경이 없으면 PR 없이 이슈에서 마무리합니다.
