...
- **마지막 확인:** 코드·빌드·실행 설정 작업은 모든 단위 완료 후 [전체 검증](../../AGENTS.md#검증과-완료)을 수행합니다. 발견된 수정은 관련 검사 후 별도 후속 커밋으로 남기며 최종 변경에 필요한 전체 검증을 마칩니다. 문서 전용 변경의 검증 예외와 사용자 커밋 금지 지시는 그대로 적용합니다.
