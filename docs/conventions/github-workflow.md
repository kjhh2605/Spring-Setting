...
- 호환성 파괴는 `type(scope)!: ...`로 표시하고 본문에 깨지는 계약과 전환 방법을 적습니다. 그 밖의 본문은 제목만으로 설명하기 어려운 이유·제약이 있을 때 작성합니다. 실제 관련 이슈가 있을 때만 연결합니다.
- 하나의 독립된 변경 목적을 하나의 커밋으로 묶습니다. 동작·필수 테스트·계약 문서를 서로 어긋난 상태로 나누지 않습니다. 문서나 테스트만의 독립 작업은 그 자체로 한 단위가 될 수 있습니다.
...
**컴파일·검증 가능한 책임 단위를 우선합니다. 500줄은 분리 가능성을 검토하는 신호이며 커밋 상한이 아닙니다.** 결합된 구현·설정·테스트를 줄 수 때문에 삭제했다가 다음 커밋에 재작성하거나, Bean 등록을 임시로 제거해 중간 상태를 만들지 않습니다. 초과가 필요하면 아래 기준으로 이유를 설명하고 해당 단위를 완결합니다.
...
- **단위의 완결성:** 코드 커밋은 컴파일 가능하고 관련 집중 테스트를 통과해야 합니다. 해당 동작에 필요한 테스트·설정·계약 문서를 함께 포함합니다. 후속 커밋의 코드가 없으면 컴파일되지 않는 상태나 실패하는 테스트를 중간 커밋으로 남기지 않습니다.
- **크기 점검:** 구현 전 예상 범위와 단위의 diff 검토 시 크기를 확인합니다. 약 500줄에 도달하거나 초과가 예상되면 분리 가능성을 점검하되 매 편집마다 줄 수를 재계산하지 않습니다. staged·unstaged·새 파일을 중복 없이 포함하고 기존 사용자 변경은 구분합니다. 산정 기준은 [PR 크기와 AI 리뷰](#pr-크기와-ai-리뷰)를 따릅니다.
- **초과 시 판단:** 독립적으로 검토·검증 가능한 책임만 분리합니다. 분리로 계약·구현·검증이 깨지거나 임시 코드 제거·재생성이 필요하면 계속 구현하기 전에 결합 범위와 이유를 짧게 설명하고 계획을 갱신합니다. 추가 사용자 승인은 필요하지 않으며, 범위·결합 이유가 바뀌지 않으면 같은 설명을 반복하지 않습니다.
