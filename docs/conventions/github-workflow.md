...
- 제목은 `type(scope): 한국어 변경 요약`으로 작성합니다. 유형은 위 작업 유형에서 실제 diff의 목적에 맞게 선택하고 `scope`는 실제 모듈·영역이며 생략할 수 있습니다. 예: `fix(user): 빈 표시 이름 등록을 차단한다`. 기존 PR 형식과 한국어 커밋 이력을 재사용합니다.
...
```text
fix(auth): 다른 발급자의 Access Token을 거부한다
