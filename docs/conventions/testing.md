...
- 코드 변경의 중간 커밋은 [단위별 집중 검사·계약 검토](workflow/checkpoints.md#checkpoints) 후 생성합니다. 각 PR의 변경·알려진 수정을 마치면 루트의 전체 검증을 수행합니다. 커밋마다 전체 검사를 반복할 필요는 없지만 필요한 집중 검사를 생략하지 않습니다. `--tests` 필터는 완료 검증에 사용하지 않습니다.
