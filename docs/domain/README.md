...
| `auth` | 카카오 OIDC 로그인, JWT·Redis RTR 세션, subject 조회 예제와 등록 이벤트 후속 처리 | 현재 없음 | `shared::error`, `shared::openapi`, `user` |
...
| 카카오 로그인·토큰·subject 조회·이벤트 소비 | [Auth](auth.md), [auth 지침](../../src/main/java/com/example/auth/AGENTS.md) | OIDC·RTR 보안, 예제 한계, 소유 모델 변환·커밋 후 처리 |
...
검증할 때는 [집중 검사 표](../conventions/testing.md)를 적용합니다. `user`의 일반 등록과 `auth`의 subject 조회는 구조 예제이고, 카카오 로그인·토큰 세션은 실제 인증 구현입니다. 새 프로젝트에서는 제품 유스케이스와 데이터 소유권에 맞춰 범위를 조정합니다.
