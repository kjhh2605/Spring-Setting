...
- 소셜 공급자 계정을 사용자와 연결하고 최초 로그인 사용자를 가입 미완료 상태로 생성합니다.
...
- `SocialAccountProvisioning.provisionKakaoUser(...)`: 카카오 회원번호로 기존 사용자를 찾거나 가입 미완료 사용자를 생성
- `ProvisionedUser`: 로그인에 필요한 사용자 식별자, 표시 이름, 가입 완료 필요 여부
...
일반 등록 사용자는 `ACTIVE`, 카카오 최초 로그인 사용자는 `PENDING_ONBOARDING` 상태로 생성합니다. 카카오 이메일이나 닉네임은 로그인 식별자로 사용하지 않으며 `KAKAO`와 카카오 회원번호 조합을 고유하게 저장합니다. 이후 로그인에서는 카카오의 최신 닉네임으로 기존 사용자 표시 이름을 덮어쓰지 않습니다.
