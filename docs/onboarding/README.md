...
현재 마이그레이션 도구는 없습니다. 운영 배포 전 스키마 준비 방식을 [백엔드 ADR](../adr/001-backend-architecture.md#영속성과-이벤트)에 따라 결정합니다. 인증과 후속 처리 지원 범위는 [Auth 문서](../domain/auth.md#책임과-범위)를 확인합니다.
