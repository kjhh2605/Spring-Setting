...
- 요청 범위의 조회·DB 정리가 없는 로컬 검사·빌드·필요한 소스 삭제·생성물 정리는 추가 승인 없이 진행합니다. 독립 변경 단위는 관련 집중 검사 후 자동으로 새 커밋을 만들며, 기본 브랜치에서는 작업용 브랜치를 먼저 생성합니다. 커밋 금지 등 명시 요청이 우선하며, 세부 기준은 [커밋 규칙](docs/conventions/github-workflow.md#커밋)을 따릅니다.
